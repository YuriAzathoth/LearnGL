...
bool glm_aabb_frustum(vec3 box[2], vec4 planes[6])
{
	...
	{
		p = planes[i];
		dp = p[0] * box[p[0] > 0.0f][0] + p[1] * box[p[1] > 0.0f][1] + p[2] * box[p[2] > 0.0f][2];
...
{
	return glm_vec3_max(box[0]) != FLT_MAX && glm_vec3_min(box[1]) != -FLT_MAX;
}
...
bool glm_aabb_sphere(vec3 box[2], vec4 s)
{
	...
	dmin = glm_pow2((s[0] - box[!(a - 1)][0]) * (a != 0)) + glm_pow2((s[1] - box[!(b - 1)][1]) * (b != 0)) +
