...
{
	glm_ortho(box[0][0], box[1][0], box[0][1], box[1][1], -box[1][2], -box[0][2], dest);
}
...
{
	glm_ortho(box[0][0], box[1][0], box[0][1], box[1][1], -(box[1][2] + padding), -(box[0][2] - padding), dest);
}
...
void glm_ortho_default_s(float aspect, float size, mat4 dest)
{
	if (aspect >= 1.0f)
	{
		glm_ortho(-size * aspect, size * aspect, -size, size, -size - 100.0f, size + 100.0f, dest);
		return;
	}

	glm_ortho(-size, size, -size / aspect, size / aspect, -size - 100.0f, size + 100.0f, dest);
}
 ...
 */
CGLM_INLINE
void glm_persp_decomp(mat4 proj,
...
{
	glm_persp_decomp(proj, &dest[0], &dest[1], &dest[2], &dest[3], &dest[4], &dest[5]);
}
 ...
 */
CGLM_INLINE
void glm_persp_decomp_x(mat4 proj, float* __restrict left, float* __restrict right)
{
 ...
 */
CGLM_INLINE
void glm_persp_decomp_y(mat4 proj, float* __restrict top, float* __restrict bottom)
{
 ...
 */
CGLM_INLINE
void glm_persp_decomp_z(mat4 proj, float* __restrict nearVal, float* __restrict farVal)
{
	float m32, m22;
