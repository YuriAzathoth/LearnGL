...
int main(int argc, char** argv)
{
	...
	if (!load_shaders_text(&vertex_shader, &fragment_shader, "data/shaders/7_diffuse"))
	{
		SDL_GL_DeleteContext(context);
		SDL_DestroyWindow(window);
		SDL_Quit();
	...
	const int uniform_light_position = glGetUniformLocation(program_diffuse, "cLightPos");
	const int uniform_light_color = glGetUniformLocation(program_diffuse, "cLightColor");
	...
	glUniform1i(glGetUniformLocation(program_diffuse, "sTexture"), 0);
	glUseProgram(0);
	...
	while (run)
	{
		...
		glUniform3fv(uniform_color, 1, light_color);
		glBindVertexArray(lamp_vao);
		glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, ebo);
		glDrawElements(GL_TRIANGLES, sizeof(cube_vertices) / sizeof(float), GL_UNSIGNED_INT, 0);
		glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, 0);
